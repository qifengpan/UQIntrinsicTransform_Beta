...
real                :: icu_per_day(8)=(/0,0,0,0,0,0,0,8/)
real                :: less_contagious
logical             :: immune_stop

!!!!!-----5. define reductions in social contacts -----
integer             :: R0change(2,24)
character*5         :: R0county(24)
logical             :: R0delay
...
character*2,allocatable         :: age_sex(:),surv_ill_label(:),surv_icu_label(:)
character*3,allocatable         :: age_sex_dur(:),temp_character(:),icu_risk_label(:)
...
!!!!!-----5. define reductions in social contacts -----
R0change = reshape((/1,6,7,12,13,18,19,25,26,32,33,39,40,46,47,53,54,60,61,67,68,74,75,81,82,88,&
...
if (control_age_sex == "age") then
        ...
        icu_risk(1:size(targert_icu_risk_index)) = 1 - (1 - icu_risk(1:size(targert_icu_risk_index))) ** (1/ real(ill_dur))
...
if (control_age_sex == "sex") then
        ...
        icu_risk(1:2) = 1 - (1 - icu_risk(1:2)) ** (1/ ill_dur)
...
do i = 1, ill_dur
        ...
        icu_risk_list%sex((i-1)*size(icu_risk)+19:i*size(icu_risk)) = 'f'
...
if (control_age_sex == "age") then
        ...
        surv_ill = surv_ill ** (1/real(ill_dur))
end if
...
if (control_age_sex == "sex") then
        ...
        surv_ill(1:2) = iol%transpr_icu_risk(targert_icu_risk_index)
        surv_ill(1:2) = 1 - (1 - surv_ill(1:2)) ** (1/ real(ill_dur))
...
if (control_age_sex == "age") then
        ...
        surv_icu= (/iol%transpr_surv_ill(targert_icu_risk_index),&
                   iol%transpr_surv_ill(targert_icu_risk_index)/)
        surv_icu = surv_ill ** (1/real(ill_dur))
end if
...
if (control_age_sex == "sex") then
        ...
        surv_icu(1:2) = iol%transpr_icu_risk(targert_icu_risk_index)
        surv_icu(1:2) = 1 - (1 - surv_icu(1:2)) ** (1/ real(ill_dur))
...
!         print *,size(temp)
        ...
        do i = 1,size(seed_ill_dur)
            seed_ill_dur(i)  = (temp_int - Date2Unixtime(seed_ill%date(i)))/86400 
        end do
        temp_int = 0
!         print *,seed_ill%cases
!         mod1 = "l"
!         mod2 = "g"
        temp = condition_and(seed_ill_dur,ill_dur,"l",seed_ill%cases,temp_int,"g")
...
!         R0_raw1 = 
    ...
    R0_daily = R0_force *lhc(2,it_ss)/real(cont_dur+real(ill_dur)*less_contagious) + &
...
!         tmp_d_new = missing
        ...
        susceptible = sum_bygroup(tmp%dist_id(tmp_count))

        at_risk = sum(susceptible)
...
!         print *,"sum(susceptible) is ",sum(susceptible)
        ...
        if (sum(susceptible) > 0 .and. nint(sum(contagious)) > 0) then
...
!             connect = transpose(connect)
            ...
            if (.not.immune_stop) then
                temp = get_index_not(tmp%t1,healthy)
...
!             print *,maxval(risk)
!             print *,minval(risk)
        !lhc(it_ss,6) is w_int
            risk = lhc(6,it_ss)*risk + (1-lhc(6,it_ss))*matmul(risk,connect)
            temp1= get_index(tmp%t1,healthy)
            temp = get_index(counties_index,tmp%dist_id(temp1))
            prob = risk(temp)
            print *,size(prob)

!             print *,"risk is",risk
!             print *,"dominator is",denominator
!             print *,"exp_infect is",exp_infect
...
!             print *,"sum sick is ",sum(sick)
            ...
            do i = 1,size(runif)
                if (runif(i) <= prob(i)) then
                    temp_int = temp_int + 1
                    revers_proj(temp_int) = temp1(i)
                end if
            end do

            initial_sick = size(revers_proj)
...
!             call sum_bygroup_distID(tmp%dist_id(revers_proj),final_count,dist_id_temp)
            ...
            if (lhc(7,it_ss) > 0 .and. Date2Unixtime(target_date) < max_date .and. initial_sick > 0)then
                ...
                if (sum(final_count) > 0) then
                    ...
                    !!!way?
                    !!!!!!!!!!!!!!!!!!!!!!!!
!                     print *,"sum of sick after shift",sum(sick)
...
!         call sum_bygroup_distID(tmp%dist_id(temp1),final_count,dist_id_temp)
        ...
        if(at_risk > 0) then
            ...
            age_sex_dur = tmp%age(temp) // tmp%sex(temp)//temp_character
            ...
            icu_risk_label = icu_risk_list%age//icu_risk_list%sex//temp_character
            
            temp1 = get_index(icu_risk_label,age_sex_dur)
            icu_risk_i = icu_risk_list%risk(temp1)
            if (.not.allocated(icu))then
                allocate(icu(size(icu_risk_i)))
                allocate(icu_count(size(icu_risk_i)))
            else
                deallocate(icu)
